...
// recordingPinchtab points the tool at a stub that keeps the raw request line
// of every request it answers, and returns the slice it appends to.
//
...
func recordingPinchtab(t *testing.T, instanceID string) *[]string {
	t.Helper()
	var seen []string
...
func TestATabIDStaysOnePathSegment(t *testing.T) {
	for _, probe := range pathProbeIDs {
		t.Run(probe.name, func(t *testing.T) {
			seen := recordingPinchtab(t, "unused")
			runBrowser(t, toolInput(t, map[string]string{"action": "close", "tab_id": probe.id}))
...
func TestAnInstanceIDStaysOnePathSegment(t *testing.T) {
	for _, probe := range pathProbeIDs {
		t.Run(probe.name, func(t *testing.T) {
			seen := recordingPinchtab(t, probe.id)
			runBrowser(t, toolInput(t, map[string]string{"action": "tabs"}))
...
func TestASnapshotFilterStaysOneQueryParameter(t *testing.T) {
	for _, probe := range queryProbeFilters {
		t.Run(probe.name, func(t *testing.T) {
			seen := recordingPinchtab(t, "unused")
			runBrowser(t, toolInput(t, map[string]string{"action": "snapshot", "filter": probe.filter}))
...
func TestAnEmptyFilterSendsNoQueryAtAll(t *testing.T) {
	seen := recordingPinchtab(t, "unused")
	runBrowser(t, toolInput(t, map[string]string{"action": "snapshot"}))
...
func TestTheRecorderCanSeeAnUnescapedValue(t *testing.T) {
	...
	base := pinchtabURL()
	if base == "" {
		t.Fatal("pinchtabURL is empty; the recorder did not install PINCHTAB_URL")
	}
